export type LootKind='Beans'|'Water'|'Bandage'|'Plank'|'Ammo';
export type LootStack={id:string;kind:LootKind;quantity:number};
export type LootCrate={id:string;name:string;x:number;y:number;items:LootStack[]};
export const LOOT_INFO:Record<LootKind,{name:string;category:string;icon:string}>={Beans:{name:'Canned beans',category:'Food',icon:'food'},Water:{name:'Drinking water',category:'Drink',icon:'water'},Bandage:{name:'Field bandage',category:'Medical',icon:'bandage'},Plank:{name:'Wooden plank',category:'Material',icon:'wood'},Ammo:{name:'Carbine rounds',category:'Ammunition',icon:'ammo'}};
export function createCrates():LootCrate[]{return [
 ...
 ].map((c,i)=>({...c,id:'crate-'+i,items:(Object.keys(LOOT_INFO) as LootKind[]).map((kind,j)=>({id:`crate-${i}-${j}`,kind,quantity:kind==='Ammo'?12:kind==='Plank'?2:1+(i+j)%3}))}));}
