...
export type Furnishing={kind:string;w:number;d:number;height:number;design:number};
...
export class Survival{
 ...
 pickupReason(t:PickupTarget){const s=this.s;if(t.blocked)return t.blocked;if(!t.furnishing)return 'This object cannot be picked up.';if(s.driving||s.dead||s.stairTravel)return 'Stand on solid ground first';if(t.floor!==s.player.floor||Math.hypot(t.x-s.player.x,t.y-s.player.y)>3||!s.clearSight(s.player,t))return 'Move closer to the object';if(t.home?.removedFurniture?.[t.floor+':'+t.solidId]||t.constructionId!==undefined&&!this.buildings.some(b=>b.id===t.constructionId))return 'The object is no longer there';return '';}
 packedDefinition(f:Furnishing){return {name:f.kind==='tv'?'TV':f.kind.replace(/^[a-z]/,c=>c.toUpperCase()),icon:'furniture:'+f.kind,w:Math.max(2,Math.min(4,Math.ceil(f.w*1.5))),h:Math.max(2,Math.min(3,Math.ceil(f.d*1.5))),weight:Math.round(f.w*f.d*8),desc:'Select PLACE IN WORLD to position this furniture. R rotates; Esc cancels without losing the item.'};}
