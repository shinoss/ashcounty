...
export class GameAudio{
 ...
 muted=true;distance=0;lastDistance=0;lastDoor=0;lastShot=0;lastSwing=0;
 ...
 surface(s:Simulation):'grass'|'pavement'|'tile'|'wood'|'gravel'{
  ...
  const lx=x-cx*46,ly=y-cy*46;
  return (ly>=19.6&&ly<=26.4)||(lx>=7&&lx<=11)?'pavement':'grass';
 }
 ...
 update(s:Simulation){const p=s.player,active=!s.paused&&!s.dead;
  ...
  if(active&&s.weapon==='bat'&&s.attackTime>this.lastSwing+.02)this.tone(240,60,.14,.12);
