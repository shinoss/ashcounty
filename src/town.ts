...
export function townBuildings(district:string,cx:number,cy:number,random:()=>number,starter:House[]):House[]{
 ...
 if(cx===0&&cy===0){result.push(...starter.map((h,i)=>({...h,loot:[...h.loot],kind:(['cottage','colonial','ranch','townhouse','colonial','cottage','ranch','townhouse'] as BuildingKind[])[i%8],floors:i%3===1?2:1,design:i%4})));add('library',14,2);add('police',31,2);add('gas',13,37);add('pharmacy',31,37);return result;}
