...
export class ProceduralWorld{
 ...
 const crates:LootCrate[]=origin?createCrates(homes):[];for(let i=origin?this.starter.length:0;i<(origin?homes.length:homes.length+(rural?0:2));i++){const home=homes[i];const x=home?home.x+1.5:ox+13+r()*29,y=home?home.y+1.7:oy+25.6;const id=`${key}:crate:${i}`;const items=(['Beans','Water','Bandage','Plank','Ammo'] as LootKind[]).filter(()=>r()>.35).map((kind,j)=>({id:`${id}:${j}`,kind,quantity:kind==='Ammo'?6+Math.floor(r()*15):1+Math.floor(r()*4)}));if(!items.length)items.push({id:id+':food',kind:'Beans',quantity:1});crates.push({id,name:home?'Household storage box':'Roadside supply crate',x,y,items});}
 ...
 if(origin)crates[0].items.push(...(['Hammer','Nails','Plank','Cloth','Scrap'] as LootKind[]).map((kind,i)=>({id:'starter-tools-'+i,kind,quantity:kind==='Hammer'?1:kind==='Plank'?60:kind==='Nails'?40:10})));
