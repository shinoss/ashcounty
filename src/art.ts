...
export function ground(localHouses:House[]=houses,worldSeed=3291,patches:Patch[]=[]){const a=canvas(OX*2,SIZE*32+300),c=a.getContext('2d')!;
 for(let x=0;x<SIZE;x++)for(let y=0;y<SIZE;y++){const patch=patches.find(p=>x>=p.x&&x<p.x+p.w&&y>=p.y&&y<p.y+p.h),road=patch?.kind==='road'||y>=21&&y<=24||x>=8&&x<=10,pavement=y===20||y===25||x===7||x===11;const id=patch?{road:1,parking:1,field:3,water:4,path:5,lawn:0,garden:3}[patch.kind]:road?1:pavement?2:0,p=iso(x,y);c.drawImage(terrainTile(id,Math.abs((x*79+y*37+worldSeed)%3)),p.x-33,p.y);if(patch?.kind==='parking'&&x%3===0&&y%4<3)line(c,[p.x-20,p.y+10],[p.x+10,p.y+25],'#b5b5a0',2);}
