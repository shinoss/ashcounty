...
export async function loadVarietyArt(){await Promise.all(['materials','furniture','outfits','survivor','city'].map(async key=>{const image=new Image();image.src='/assets/town-variety/'+key+(['survivor','city'].includes(key)?'-v3.png':'-v2.png');await image.decode();atlases.set(key,image);}));}
